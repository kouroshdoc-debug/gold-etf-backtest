...
def trade_log(df,v,thr=10,trend=200,dev=-.002,cost=.00125,start=0,end=None):
    x,e=signals(df,v,thr,trend,dev);end=len(df) if end is None else end;pos=False;ent=None;rows=[]
...
def metrics(t,years):
    ...
    eq=(1+t["return"]).cumprod();dd=eq/eq.cummax()-1;g=t.loc[t["return"]>0,"return"].sum();l=-t.loc[t["return"]<0,"return"].sum()
...
def random_benchmark(df,t,n,seed,cost):
    ...
    for _ in range(n):
        ...
        vals.append(np.prod(1+np.array(rr))-1)
...
def run(df,cfg):
    ...
    for v in [f"V{i}" for i in range(6)]:
        t=trade_log(df,v,**defaults,cost=b["one_way_cost"]);m=metrics(t,years);m.update(random_benchmark(df,t,b["random_trials"],b["seed"],b["one_way_cost"]));m.update({"version":v,"rsi_threshold":10,"trend_window":200});summary.append(m)
    ...
    for v in ["V2","V3","V4","V5"]:
        for th in b["rsi_thresholds"]:
            for tw in b["trend_windows"]:
                t=trade_log(df,v,th,tw,b["v5_last_close_deviation"],b["one_way_cost"]);sens.append({"version":v,"rsi_threshold":th,"trend_window":tw,**metrics(t,years)})
    cut=int(len(df)*(1-b["oos_fraction"]));oos=[]
    for v in [f"V{i}" for i in range(6)]:oos.append({"version":v,**metrics(trade_log(df,v,**defaults,cost=b["one_way_cost"],start=cut),(len(df)-cut)/252)})
    ...
    for end in range(train,len(df),test):
        ...
        for v in ["V2","V3","V4","V5"]:
            cand=[]
            for th in b["rsi_thresholds"]:
                for tw in b["trend_windows"]:cand.append((metrics(trade_log(df,v,th,tw,b["v5_last_close_deviation"],b["one_way_cost"],0,end),end/252)["cagr"],th,tw))
            _,th,tw=max(cand,key=lambda z:z[0]);wf.append({"version":v,"test_start":int(df.date.iloc[end]),"test_end":int(df.date.iloc[stop-1]),"chosen_rsi":th,"chosen_sma":tw,**metrics(trade_log(df,v,th,tw,b["v5_last_close_deviation"],b["one_way_cost"],end,stop),(stop-end)/252)})
