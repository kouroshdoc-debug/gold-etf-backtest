...
def equity_curve(df,t,cost):
    curve=pd.Series(1.0,index=df.index,dtype=float);capital=1.0;cursor=0
    for row in t.itertuples():
        curve.iloc[cursor:row.entry_i]=capital;basis=row.entry*(1+cost)
    ...
    curve.iloc[cursor:]=capital
    return curve
def metrics(t,years,df=None,cost=.00125):
    if t.empty:return {"trades":0,"total_return":0,"cagr":0,"max_drawdown":0,"win_rate":0,"profit_factor":0,"exposure":0}
    eq=equity_curve(df,t,cost) if df is not None else (1+t["return"]).cumprod();dd=eq/eq.cummax()-1;g=t.loc[t["return"]>0,"return"].sum();l=-t.loc[t["return"]<0,"return"].sum()
